...
if groups $USER | grep -q "\b${GROUP_NAME}\b"; then
    ...
    add_alias "wmods" "cd ${GAMEDIR}/ugc_mods/${CLUSTER_NAME}/Master/content/${G                                                                                                             AMEID}" "Change your pwd to the ${CLUSTER} server's ugc_mods directory storing n                                                                                                             ewer workshop mods."
    add_alias "mods" "cd ${GAMEDIR}/mods" "Change your pwd to the ${CLUSTER} ser                                                                                                             ver's mods directory."
    add_alias "sboot" "cd ${GAMEDIR}/boot" "Change your pwd to the ${CLUSTER} se                                                                                                             rver's boot directory."

    echo ""

    for SHARD in ${SHARDS[@]}; do
        add_alias "${SHARD,,}" "cd ${CLUSTER_PATH}/${CLUSTER}/${SHARD}" "Change                                                                                                              your pwd to the ${CLUSTER} server's ${SHARD} shard directory."
        add_alias "c_${SHARD,,}" "screen -r ${SESSION_OWNER}/${SHARD_SESSION_PRE                                                                                                             FIX}${SHARD}" "Open the console for the ${SHARD} shard."
    done

    # TODO: Make this display only the server's processes.
    alias pcores='for pid in $(ps -a -o pid=); do source_file=$(ps -p $pid -o ar                                                                                                             gs=); affinity=$(taskset -pc $pid 2>/dev/null); \
                  echo "PID: $pid, Source File: $source_file, Affinity: $affinit                                                                                                             y"; done'
    ...
    add_alias "c_start" "systemctl start ${SERVICE}.service" "Updates the game a                                                                                                             nd starts the shards. The same as running "systemctl start" on the server's serv                                                                                                             ice."
